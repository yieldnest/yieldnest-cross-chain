...
abstract contract BatchScript is Script {
    ...
    //     "baseGas": 0,  // Gast costs not related to the transaction execution (signature check, refund
    // payment...)
    ...
    //     "nonce": 0,  // Nonce of the Safe, transaction cannot be executed until Safe's nonce is not equal to
    // this nonce
    //     "contractTransactionHash": "string",  // Contract transaction hash calculated from all the field
    //     "sender": "<checksummed address>",  // Owner of the Safe proposing the transaction. Must match one of
    // the signatures
    ...
    modifier isBatch(address safe_) {
        ...
        if (chainId == 1) {
            ...
            SAFE_MULTISEND_ADDRESS = 0x0B1795ccA8E4eC4df02346a082df54D437F8D9aF;
    ...
    function _getSafeAPIEndpoint(address safe_) private view returns (string memory) {
        if (keccak256(bytes(SAFE_API_BASE_URL)) == keccak256(bytes(""))) {
            revert("Safe API base URL not set");
        }
