 ...
 * ```
 */
contract BridgeAssetERC20Sample is BaseData {
    using OptionsBuilder for bytes;

    // Amount to bridge
    uint256 public constant BRIDGE_AMOUNT = 0.01 ether;

    function run() external {
        uint256 sourceChainId = block.chainid;
        uint256 baseChainId = 1; // mainnet

        // Load deployment config
        string memory json =
            vm.readFile(string.concat("deployments/ERC20Sample-2-", vm.toString(baseChainId), "-v0.0.1.json"));
 ...
 * Usage:
 * ```
 * forge script script/commands/BridgeAsset.s.sol:BridgeAssetYnRWAx --rpc-url <RPC_URL> --broadcast
 * ```
 */
contract BridgeAssetYnRWAx is BaseData {
    using OptionsBuilder for bytes;
    ...
    function run() external {
        ...
        // Load deployment config
        string memory json =
            vm.readFile(string.concat("deployments/ynRWAx-", vm.toString(baseChainId), "-v0.0.1.json"));
